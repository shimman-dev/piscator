...
func TestGetRepos(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := GetRepos(client, sleeper, tt.name, tt.token, tt.isSelf, tt.isOrg, tt.isForked, tt.makeFile)
			if (err != nil) != tt.wantError {
				t.Errorf("GetRepos() error = %v, wantError %v", err, tt.wantError)
			}

			if tt.name == "forked repos" {
				filteredJSON, err := GetRepos(client, sleeper, tt.name, tt.token, tt.isSelf, tt.isOrg, tt.isForked, tt.makeFile)
				if (err != nil) != tt.wantError {
					t.Errorf("GetRepos() error = %v, wantError %v", err, tt.wantError)
				}
